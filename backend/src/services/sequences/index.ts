...
const serviceLogger = createLogger({ module: 'sequences-service' });

const app = express();

const SERVICE_PORT = parseInt(process.env.SERVICE_PORT || '3005', 10);
const SERVICE_NAME = process.env.SERVICE_NAME || 'sequences';
...
// Mount at root (gateway strips /api/v1/sequences prefix)
...
const server = app.listen(SERVICE_PORT, config.host, () => {
  serviceLogger.info(
    ...
    `🔄 Sequences Service listening on ${config.host}:${SERVICE_PORT}`
  );
});
...
const shutdown = async (signal: string) => {
  ...
  server.close(() => {
    serviceLogger.info('Sequences Service HTTP server closed');
    process.exit(0);
  });
